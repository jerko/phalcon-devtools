...
class Mbit extends Component
{
    ...
    public function build()
    {
        ...
        $path = realpath('.') . DIRECTORY_SEPARATOR.'config/';
     ...
     * @return string
     */
    private function _captureMbitInput($var, $fields, $useGetSetters, $identityField)
    {
        $code = '';
        foreach ($fields as $field => $dataType) {
            if ($field == $identityField) {
    ...
    /**
     * Generate controller using scaffold mbit
     *
     * @param string $path
     * @param array  $options
     */
    private function _makeController($path, $options)
    {
        ...
        $path = $options['templatePath'] . '/mbit/no-forms/Controller.php';
        ...
        $code = str_replace('$assignInputFromRequestCreate$', $this->_captureMbitInput($options['singular'], $options['dataTypes'], $options['genSettersGetters'], $options['identityField']), $code);
    ...
    private function makeView($path, $options, $type)
    {
        ...
        $templatePath = $options['templatePath'] . '/mbit/no-forms/views/' .$type. '.phtml';
    ...
    private function makeViewVolt($path, $options, $type)
    {
        ...
        $templatePath = $options['templatePath'] . '/mbit/no-forms/views/' .$type. '.volt';
        if (!file_exists($templatePath)) {
            throw new BuilderException("Template '" . $templatePath . "' does not exist");
        }
    ...
    private function _makeViewSearch($path, $options)
    {
        ...
        $templatePath = $options['templatePath'] . '/mbit/no-forms/views/search.phtml';
        if (!file_exists($templatePath)) {
            throw new BuilderException("Template '" . $templatePath . "' does not exist");
        }
    ...
    private function _makeViewSearchVolt($path, $options)
    {
        ...
        $templatePath = $options['templatePath'] . '/mbit/no-forms/views/search.volt';
        if (!file_exists($templatePath)) {
            throw new BuilderException("Template '" . $templatePath . "' does not exist");
        }
